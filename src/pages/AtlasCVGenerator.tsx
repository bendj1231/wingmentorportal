...
const AtlasCVGenerator: React.FC<AtlasCVGeneratorProps> = ({ onBack, userProfile }) => {
  ...
  useEffect(() => {
    const fetchCVData = async () => {
      ...
      try {
        // Fetch pilotProfiles document
        const profileSnap = await getDoc(doc(db, 'pilotProfiles', userProfile.uid));
        ...
        const achQuery = query(collection(db, 'achievements'), where('userId', '==', userProfile.uid));
        const achSnap = await getDocs(achQuery);
        const certEntries: EducationEntry[] = [];
        achSnap.forEach((d) => {
          const data = d.data();
          if (data.type === 'certification' || data.category === 'certification') {
            certEntries.push({
              id: base.education.length + certEntries.length + 1,
              degree: data.title || data.name || 'Certification',
              institution: data.issuer || 'WingMentor',
